    ...
    syllabusData.forEach(topic => {
        ...
        if (topic.exercises && topic.exercises.length > 0) {
            ...
            uniqueExercises.forEach(exe => {
                ...
                // MÃ HÓA BẢO VỆ DẤU NHÁY ĐƠN (Khắc phục lỗi không click được)
...
window.openExercise = function(encodedData) {
    try {
        ...
        if (exe.module_type === "learning") {
            ...
            exe.activities.forEach(act => {
                const type = act.type || act.activity_type; 
                // Kiểm tra nếu là thẻ từ vựng/lý thuyết
                if (type === "Học Từ Vựng (Vocabulary)" || act.content.word) {
                    hasContent = true;
                    contentDiv.innerHTML += `
                        <div class="theory-card">
                            <div class="theory-title">📌 ${act.content.word || act.content.title || ""}</div>
                            <div class="theory-desc">${act.content.meaning || act.content.description || ""}</div>
                        </div>
                    `;
                }
            });
            ...
            if (!hasContent) {
                contentDiv.innerHTML = `<p style="text-align:center; color:#64748b;">Nội dung lý thuyết đang được cập nhật. Thầy Nam sẽ sớm bổ sung!</p>`;
            }
            document.getElementById("learning-modal").style.display = "flex";
        ...
        alert("⚠️ Đã xảy ra lỗi khi đọc dữ liệu bài học. Vui lòng báo lại với Thầy Nam!");
        console.error(error);
    }
};
...
function renderCurrentQuestion() {
    ...
    let html = `<div style="margin-bottom: 15px;">
        <span style="background:#e0f2fe; color:#0284c7; padding: 5px 12px; border-radius: 12px; font-size: 0.85rem; font-weight:bold;">Câu ${currentQIndex + 1} / ${currentPracticeActs.length} - ${type || 'Bài tập'}</span>
    </div>`;
